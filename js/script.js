...
function mulai() {
  ...
  timer(); 
  mulaiGame();
  musikLatar.currentTime = 0; // Reset ke awal musik
  musikLatar.play(); // Mulai memutar musik latar
  musikLatar.volume = 0.5; // Set volume ke 50%
}
...
function timer() {
  timerId = setInterval(() => {
    ...
    if (waktu <= 0) {
      ...
      gameBerjalan = false;
      alert("Waktu habis & terimakasih sudah memainkan game Whack A Bear Hararu! Skor akhir Anda: " + document.querySelector(".papan-skor").textContent);
      resetGame(); 
    }
  }, 1000); 
}
